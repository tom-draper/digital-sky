...
function createGrid(h, w) {
  var grid = __spreadArray([], Array(h), true).map(function (e) {
    return Array(w);
  });
  return grid;
}
...
function colourSky(grid, skyConfig) {
  ...
  var startColour = __spreadArray(
    __spreadArray([], skyConfig.properties.colour, true),
...
function addCloudToSky(grid, x, y, colour, layer) {
  var _a = pixelHasType(grid[y][x], "cloud" + layer),
    hasCloud = _a[0],
    idx = _a[1];
  if (hasCloud) {
...
function createCloud(grid, layers) {
  var start = createCloudBase(
    grid,
    __spreadArray(
      __spreadArray([], layers[0].colour, true),
      [layers[0].opacity],
      false
    ),
  ...
  for (var i = 1; i < layers.length; i++) {
    addCloudLayer(
      ...
      i,
      __spreadArray(
        __spreadArray([], layers[i].colour, true),
        [layers[i].opacity],
        false
      ),
...
function addSunsetToSky(grid, x, y, colour) {
  var _a = pixelHasType(grid[y][x], "sunset"),
    hasSunset = _a[0],
    idx = _a[1];
  if (hasSunset) {
    grid[y][x][idx].colour = combineColours(colour, grid[y][x][idx].colour);
  ...
  while (toPaint.length > 0) {
    ...
    if (scale > 0) {
      ...
      sunsetSpread(
        ...
        toPaint,
        seen,
        layerConfig.colourMutationSpeed
      );
    }
  }
}
...
function fullMoon(grid, toPaint, moonConfig) {
  var colour = __spreadArray(
    __spreadArray([], moonConfig.properties.colour, true),
    [1],
    false
  );
  while (toPaint.length > 0) {
    var _a = nextPixel(toPaint),
      x = _a[0],
      y = _a[1];
