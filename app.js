...
function nextPixelColourless(toPaint) {
...
function createCloudBase(grid, startColour, sizeRange, pH, pV) {
    ...
    var cloudSize = 1;
    while (toPaint.length > 0) {
        var _a = nextPixel(toPaint), x = _a[0], y = _a[1], colour = _a[2];
...
function createSunsetLayer(grid, layerConfig) {
    ...
    var seen = new TupleSet();
    var toPaint = [];
    var colour = [253, 94, 83, layerConfig.maxOpacity];
    var start = [randInt(0, w - 1), h - 1];
    toPaint.push([start[0], start[1], layerConfig.colour]);
    seen.add(start);
    var scale;
    while (toPaint.length > 0) {
        var _a = nextPixel(toPaint), x = _a[0], y = _a[1], colour_1 = _a[2];
        scale =
            1 -
                warpedDistance(x, y, start[0], start[1], layerConfig.xStretch, layerConfig.yStretch) /
                    maxD;
        if (scale > 0) {
            colour_1[3] = layerConfig.maxOpacity * scale;
            addSunsetToSky(grid, x, y, colour_1);
            sunsetSpread(x, y, colour_1, toPaint, seen, layerConfig.colourMutationSpeed);
        }
    }
}
...
function fullMoon(grid, toPaint, moonConfig) {
    ...
    while (toPaint.length > 0) {
        var _a = nextPixelColourless(toPaint), x = _a[0], y = _a[1];
...
var config = presets_1.presetLateEvening3;
